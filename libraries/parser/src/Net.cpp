...
/*                      class RdPE                         */
/***********************************************************/
net::net(const char *f, const char *Formula_trans, const char *Int_trans) {
...
net::net(const char *f, const set<string> & f_trans) {
 ...
 * ------------------------------*/
 void net::setListObservable(const set<string> & list_t) {
    ...
    for (set<string>::const_iterator i=list_t.begin();i!=list_t.end();i++) {
        ...
        if (pos==-1) {
            cout<<"Error: "<<*i<<" transition not found!"<<endl;
        } else Formula_Trans.insert(pos);
    }
 }
...
bool net::addPostQueue(const string &place, const string &trans,
                       int valuation) {
  int p, t;
  map<string, int>::const_iterator pi = placeName.find(place);
  if (pi == placeName.end() || !places[pi->second].isQueue())
...
bool net::addInhibitor(const string &place, const string &trans,
                       int valuation) {
  int p, t;
  map<string, int>::const_iterator pi = placeName.find(place);
  if (pi == placeName.end())
...
bool net::addPreAuto(const string &place, const string &trans,
...
bool net::addPostAuto(const string &place, const string &trans,
                      const string &valuation) {
  int p, t, v;
  map<string, int>::const_iterator pi = placeName.find(place);
  if (pi == placeName.end() || places[pi->second].isQueue())
